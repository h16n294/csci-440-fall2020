...
public class Homework extends DBTest {
    ...
    void selectAllArtistsWithMoreThanOneAlbum(){
        List<Map<String, Object>> results = executeSQL("SELECT artists.Name, COUNT(*) as AlbumCount FROM albums JOIN artists on albums.ArtistId = artists.ArtistId GROUP BY albums.ArtistId HAVING (AlbumCount > 1)");
    ...
    void selectTheTrackAndAlbumAndArtistForAllTracksLongerThanSixMinutes() {
        List<Map<String, Object>> results = executeSQL("SELECT tracks.Name as TrackName, albums.Title as AlbumTitle, artists.Name as ArtistsName FROM tracks " + "JOIN albums on tracks.AlbumId = albums.AlbumId JOIN artists on albums.ArtistId = artists.ArtistId WHERE (Milliseconds > 360000)");
